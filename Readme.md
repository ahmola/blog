...
# 마이크로서비스 아키텍쳐 설계
  ...
  - 백엔드(API Gateway : Port 8080, Netflix Eureka Server : Port 8761 Microservice : 8081 ~ 8084)
        ...
        2. API Gateway는 Eureka Server에서 Eureka Server가 제공하는 Service Discovery를 통해 알맞은 서비스를 찾아준다.

        3. API Gateway는 JWT Auth Service를 통해 사용자의 자격증명을 생성/검증한다. 사용자 자격 증명을 위해 로그인 관련된 주소들은 자격 증명없이 열어둔다.

        4. 사용자 인증은 Json Web Token을 발급하는 방식으로 클라이언트(React 페이지)는 발급받은 토큰을 사용하여 요청한 서비스를 제공받는다(추후 OAuth2도 추가할 예정).

        5. 각 마이크로서비스는 도커 이미지로 Docker Compose를 통해 일괄 컨테이너화하여 EC2 인스턴스에서 운영된다.

        6. 각 마이크로서비스는 연계되는 사항들에 대해서 Kafka의 비동기 메시지를 통해 주고받으며 작업을 처리한다(인증 서버에서 유저가 추가되면 유저 서비스에 해당 유저를 추가할 것을 알림).


  - 메시지 브로커 서버(Kafka, Port: 9092)

        1. 비동기 메시징 시스템을 제공하며, 마이크로서비스 간 비동기 통신에 사용된다.
   
        2. RabbitMQ와 달리, 비메모리 시스템에 높은 처리량을 감당할 수 있음
   
  - 캐시 서버(Redis, Port: 6379)

        1. 인메모리 시스템의 캐시 서버로, 자주 조회되면서 불변성인 데이터들을 저장하여 높은 성능을 보장한다(데이터베이스 부하 down, 응답 속도 up).
   
        2. 세션 저장소로도 사용하여, 사용자의 로그인 정보 등을 저장하는데도 사용된다. Spring Session과 함께 쓰인다.
...
## 개선점 및 느낀점
    ...
    - yaml파일이 너무 많아서 이번에 config server를 새로 추가했음

    - 근데 config server를 좀 더 세밀하게 분리하려고 햇는데 안되서 아마 나중에 해야될 것 같다.

    - jwt관련 설정도 그렇고, db관련 설정도 잘게 쪼개고 싶은데 yml -> yml로 이어지게 구상중

    - 그리고 이러한 설정을 즉각 적용 가능하게 하려면 RabbitMQ나 Kafka가 필요하다고 함(추후 추가할수도)
    
    - 내일 config관련 설정이랑 서비스들을 마저 작성

    - 그리고 게이트웨이에서는 jwt 검증, 인증 서버에서는 jwt 생성과 사용자 인증으로 나눌 예정

    - 원래는 게이트웨이에서는 모두 처리할려고 햇는데 그러면 인증 서버를 두는 의미도 없고

    - 찾아보니 존재하는 토큰들만 검증하여 서비스 라우팅을 해주는게 게이트웨이고

    - 자격 증명으로 토큰을 만들고 사용자를 인증하는 프로세스는 인증서버에서 진행한다고 함

    - 그러니까 게이트웨이는 유효한지 판별만 하고 나머지 생성이나 보안 컨텍스트 관리 등을 인증 서버가 관리한다.
    
    - 그리고 아무래도 역할을 나눠야될 것 같다. 역할에 따라서 제공하는 서비스가 달라져서 아마 이것도 다시 짜야될 듯 싶다.
